...
class Client(Base):
    ...
    source               = Column(String(50), default="mikey-cdmx")
...
async def upsert_client(data: ClientCreate, db: AsyncSession = Depends(get_db)):
    ...
    if client:
        ...
        client = Client(
            ...
            source=data.source or "mikey-cdmx", market=data.market or "cdmx",
        )
        db.add(client)
    await db.commit()
...
async def create_booking(data: BookingCreate, db: AsyncSession = Depends(get_db)):
    ...
    booking = Booking(
        ...
        is_vip=data.is_vip or False, market=data.market or "cdmx",
    )
    db.add(booking)
    await db.commit()
...
class CheckoutRequest(BaseModel):
    ...
    description:         Optional[str]  = "Mexico City Black Car Service"
        ...
        session = stripe.checkout.Session.create(
            ...
            line_items=[{
                "price_data": {
                    ...
                    "product_data": {
                        "name": data.description or "Mexico City Black Car Service",
                    },
                },
                "quantity": 1,
            }],
            ...
            success_url="https://mexicocityblackcar.com?payment=success",
            cancel_url="https://mexicocityblackcar.com?payment=cancelled",
        )
